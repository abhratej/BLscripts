program blprops
    ...
    REAL, ALLOCATABLE :: vort_mag(:), coords(:,:), velocity(:,:), vel_mag(:)
    ...
    do airfoil_point_num = 1, airfoilpoints
        ...
        ALLOCATE(vort_mag(ngridpoints), coords(ngridpoints,3), velocity(ngridpoints,3), vel_mag(ngridpoints))
        ...
        do i = 1, ngridpoints
            ...
            read (fu,*,iostat=io) temp, temp, temp, temp, temp, temp, temp, temp, temp, temp, temp, velocity(i,1), velocity(i,2), & 
                                    velocity(i,3), temp, temp, temp, vel_mag(i), vort_mag(i), coords(i,1), coords(i,2), coords(i,3)
        end do
        ...
        do i = 1, ngridpoints-1
            bl_index = i
            if ((vort_mag(i)/52.1573).lt.0.001 .and. vort_mag(i).gt.vort_mag(i+1)) exit
        end do
        ...
        DEALLOCATE(vort_mag, coords, velocity, vel_mag)
